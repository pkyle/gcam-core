...
module_gcamusa_trn_scenarios_xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    Trn_Shrwt_DECARB <- filter(trn_tech_scenarios,
                           !is.na(technology),
    ...
    L254.GlobalTechShrwt_trn_LowTech <- filter(Trn_Shrwt_DECARB, scenario == "LowTech")
    L254.GlobalTechShrwt_trn_AdvTech <- filter(Trn_Shrwt_DECARB, scenario == "HiTech")

    # 2. Global technology share-weight interpolation rules
    GlobalTechInterpTo_DECARB <- filter(trn_tech_scenarios, variable %in% c("from.year", "to.value", "to.year", "interpolation.function")) %>%
    ...
    L254.GlobalTechInterpTo_LowTech <- filter(GlobalTechInterpTo_DECARB, scenario == "LowTech")
    L254.GlobalTechInterpTo_AdvTech <- filter(GlobalTechInterpTo_DECARB, scenario == "HiTech")

    # 3. Energy intensity adjustment
    Coef_DECARB <- filter(trn_tech_scenarios,
                          !is.na(adj_factor),
    ...
    L254.StubTranTechCoef_LowTech <- filter(L254.StubTranTechCoef_DECARB, scenario == "LowTech")
    L254.StubTranTechCoef_AdvTech <- filter(L254.StubTranTechCoef_DECARB, scenario == "HiTech")

    L254.StubTranTechCoef_USA_LowTech <- filter(L254.StubTranTechCoef_USA_DECARB, scenario == "LowTech")
    L254.StubTranTechCoef_USA_AdvTech <- filter(L254.StubTranTechCoef_USA_DECARB, scenario == "HiTech")

    # 4. Capital costs adjustment
    Cost_DECARB <- filter(trn_tech_scenarios,
                          !is.na(adj_factor),
                          variable == "input.cost") %>%
      select(scenario, supplysector, tranSubsector = subsector, stub.technology = technology, year = value, adj_factor) %>%
      mutate(year = as.numeric(year))
    ...
    L254.StubTranTechCost_LowTech <- filter(L254.StubTranTechCost_DECARB, scenario == "LowTech")
    L254.StubTranTechCost_AdvTech <- filter(L254.StubTranTechCost_DECARB, scenario == "HiTech")

    L254.StubTranTechCost_USA_LowTech <- filter(L254.StubTranTechCost_USA_DECARB, scenario == "LowTech")
    L254.StubTranTechCost_USA_AdvTech <- filter(L254.StubTranTechCost_USA_DECARB, scenario == "HiTech")
