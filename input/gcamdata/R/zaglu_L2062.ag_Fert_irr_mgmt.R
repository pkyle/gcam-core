...
module_aglu_L2062.ag_Fert_irr_mgmt <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    A_Fodderbio_chars %>%
      ...
      mutate(coefficient = round((aglu.BIO_GRASS_N_FERT_IO_GNM2 * CONV_G_KG / aglu.BIO_GRASS_YIELD_KGCM2  # Convert from application per unit area to per unit carbon
                            * aglu.CCONTENT_CELLULOSE * (1 - WaterContent))                             # Convert from carbon to wet biomass
                            / (aglu.BIO_ENERGY_CONTENT_GJT * CONV_KG_T), aglu.DIGITS_CALOUTPUT)) ->     # Convert from biomass to energy
      bio_grass_coef

    # Calculate fertilizer coefficients for tree bioenergy crops
    A_Fodderbio_chars %>%
      filter(GCAM_commodity == "biomassTree") %>%
      mutate(coefficient = round((aglu.BIO_TREE_N_FERT_IO_GNM2 * CONV_G_KG / aglu.BIO_TREE_YIELD_KGCM2    # Convert from application per unit area to per unit carbon
                                  * aglu.CCONTENT_CELLULOSE * (1 - WaterContent))                         # Convert from carbon to wet biomass
                                 / (aglu.BIO_ENERGY_CONTENT_GJT * CONV_KG_T), aglu.DIGITS_CALOUTPUT)) ->  # Convert from biomass to energy
      bio_tree_coef
    ...
    L2052.AgCost_bio_irr_mgmt %>%
      ...
      L2062.AgCoef_NFert_bio_irr_mgmt

    # Add phosphorus assumption based on default P:N ratio
    L2062.AgCoef_PFert_bio_irr_mgmt <- L2062.AgCoef_NFert_bio_irr_mgmt %>%
      mutate(minicam.energy.input = aglu.P_FERT_NAME,
             coefficient = coefficient * aglu.BIO_P_TO_N_RATIO)

    L2062.AgCoef_Fert_bio_irr_mgmt <- bind_rows(L2062.AgCoef_NFert_bio_irr_mgmt, L2062.AgCoef_PFert_bio_irr_mgmt)
    ...
    L2052.AgCost_ag_irr_mgmt %>%
      ...
      left_join(L2062.AgCoef_Fert_ag_irr_mgmt,
                by = c("region", "AgSupplySector", "AgSupplySubsector", "AgProductionTechnology", "year", "minicam.energy.input")) %>%
      ...
      mutate(FertCost = coefficient * aglu.N_FERT_PRICE * gdp_deflator(1975, aglu.N_FERT_PRICE_YEAR) * CONV_KG_T / CONV_NH3_N) %>%
    ...
    L2052.AgCost_bio_irr_mgmt %>%
      # Note: using left_join because there are instances with cost but no fertilizer use
      left_join(L2062.AgCoef_NFert_bio_irr_mgmt,
    ...
    L2062.AgCoef_Fert_bio_irr_mgmt %>%
      ...
      add_units("kgN per GJ and kgP per GJ") %>%
