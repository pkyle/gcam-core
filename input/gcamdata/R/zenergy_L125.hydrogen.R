...
module_energy_L125.hydrogen <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    year <- value <- technology <- capital.overnight <- coal_IGCC_CCS <- coal_IGCC <-
      sector.name <- subsector.name <- IGCC_CCS_no_CCS_2020_ratio <- efficiency <-
      IGCC_CCS <- IGCC_no_CCS <- with_CCS <- without_CCS <- CCS_add_cost <- `2100` <-
      `2040` <- `2020` <- max_improvement <- CCS_sub_eff <- notes <- minicam.energy.input <-
    ...
    nonFuelLCOE_elec %>%
      ...
      select(sector.name, subsector.name, IGCC_CCS_no_CCS_2020_ratio) -> elec_IGCC_2020_cost_ratio
    ...
    L223.GlobalTechEff_elec %>%
      ...
      mutate(IGCC_CCS_no_CCS_2020_ratio = IGCC_CCS / IGCC_no_CCS) %>%
      select(sector.name, subsector.name, IGCC_CCS_no_CCS_2020_ratio) -> elec_IGCC_2020_eff_ratio


    elec_IGCC_2020_eff_ratio %>%
      filter(subsector.name == "biomass") -> elec_IGCC_2020_eff_ratio_bio

    elec_IGCC_2020_eff_ratio %>%
      filter(subsector.name == "coal") -> elec_IGCC_2020_eff_ratio_coal

    # B. Calculate maximum future improvement (2100/energy.H2A_CURRENT_YEAR) of CCS for biomass and coal IGCC electricity technologies
    #    Costs:
    nonFuelLCOE_elec %>%
      filter(technology %in% c("coal (IGCC)", "coal (IGCC CCS)","biomass (IGCC)", "biomass (IGCC CCS)"),
    ...
    L223.GlobalTechEff_elec %>%
     ...
     # YZ 3/11/2025 note: the value of CONV_GJ_KGH2 is actually converting KG H2 to GJ, should have been named as
     # CONV_KGH2_GJ following the same naming conventions as other conversion constants in gcamdata
     H2A_prod_cost %>%
    ...
    add_coal_and_bio %>% # calculate the incremental cost of CCS
      ...
      arrange(sector.name, subsector.name, technology, minicam.non.energy.input,year) %>%
      group_by(sector.name, subsector.name, technology, minicam.non.energy.input) %>%
      mutate(improvement_rate = (1 - max_improvement) ^ (1 / (2100 - energy.H2A_CURRENT_YEAR)) - 1,
             min_cost = value[year == energy.H2A_CURRENT_YEAR]*(1 - max_improvement),
             cost = if_else(year <= energy.H2A_CURRENT_YEAR,value[year == energy.H2A_CURRENT_YEAR],
                            value[year == energy.H2A_CURRENT_YEAR] * (1 + improvement_rate) ^ (year - energy.H2A_CURRENT_YEAR)), #apply calculated CAGR from above to calculate cost declination pathway
    ...
    H2A_prod_coef_conv %>%
      ...
      mutate(value = 1 / value,
             ...
             improvement_to_2040 = (value[year == 2040] - value[year == energy.H2A_CURRENT_YEAR]) / value[year == energy.H2A_CURRENT_YEAR],
             improvement_rate = (1 + improvement_to_2040) ^ (1 / (2040 - energy.H2A_CURRENT_YEAR)) - 1) %>%
      ungroup() -> H2A_eff_improvement



    # Add 2020 value for coal w/o CCS and bio w/CCS, using same approach as for costs described above.
    ...
    existing_coal_bio_eff %>%
      mutate(value = if_else(subsector.name == "biomass" & !(minicam.energy.input %in% c('water_td_ind_C','water_td_ind_W')),value * elec_IGCC_2020_eff_ratio_bio$IGCC_CCS_no_CCS_2020_ratio, #use efficiency ratios from electricity to derive coefficients for bio CCS, coal w/o CCS
                             if_else(subsector.name == "coal" & !(minicam.energy.input %in% c('water_td_ind_C','water_td_ind_W')),value / elec_IGCC_2020_eff_ratio_coal$IGCC_CCS_no_CCS_2020_ratio,
                                     value)),
             technology = if_else(subsector.name == "coal", "coal chemical",
                                  if_else(subsector.name == "biomass", "biomass to H2 CCS",
                                          NA_character_)),
             ...
             improvement_rate = (1 + improvement_to_2040) ^ (1 / (2040 - energy.H2A_CURRENT_YEAR)) - 1)-> add_coal_and_bio_eff
    ...
    H2A_eff_improvement %>%
      ...
      mutate(max_improvement = round(improvement_to_2040 + 0.1, 2),
             ...
             max_improvement = if_else( minicam.energy.input %in% c( "water_td_ind_W", "water_td_ind_C" ),    #     Water coefs see no improvement past 2040 H2A assumptions
                                        improvement_to_2040, max_improvement ) ) -> H2A_eff_add_2020_techs


    H2A_eff_add_2020_techs %>%
    ...
    H2A_eff_add_2020_techs %>%
      #      Onsite electrolysis max improvement = central electrolysis max improvement - 1%
    ...
    H2A_eff_fix_improv %>%
      arrange(sector.name, subsector.name, technology, minicam.energy.input,year) %>%
      group_by(sector.name, subsector.name, technology, minicam.energy.input) %>%
      mutate(value = case_when(year<energy.H2A_CURRENT_YEAR ~ value[year == energy.H2A_CURRENT_YEAR],
                               (year>=energy.H2A_CURRENT_YEAR) ~ value[year == energy.H2A_CURRENT_YEAR]*(1 + improvement_rate) ^ (year - energy.H2A_CURRENT_YEAR),
                               year >= 2040 ~ value[year == 2040]*(1 + improvement_rate_post_2040)^(year - 2040)),
             value = case_when(technology == 'coal chemical' & year>energy.H2A_CURRENT_YEAR~value[year == energy.H2A_CURRENT_YEAR]*(1 + improvement_rate) ^ (year - energy.H2A_CURRENT_YEAR),
                               TRUE~value),
             improve_max = case_when( ( year >= 1975 ) ~ ( value[ year == energy.H2A_CURRENT_YEAR] * ( 1 + max_improvement ) ) ),
    ...
    add_coal_and_bio_eff %>%
      ...
      filter(year==energy.H2A_CURRENT_YEAR) %>%
      arrange(sector.name, subsector.name,technology,minicam.energy.input,year) %>%
      group_by(sector.name, subsector.name,minicam.energy.input) -> add_coal_and_bio_eff_2020

    add_coal_and_bio_eff_2020 %>%
    ...
    ccs_eff_loss %>%
      ...
      arrange(sector.name, subsector.name, technology, minicam.energy.input,year) %>%
      group_by(sector.name, subsector.name, technology, minicam.energy.input) %>%
      mutate(max_improvement = max_improvement[year == energy.H2A_CURRENT_YEAR],
             improvement_rate = (1 - max_improvement) ^ (1 / (2100 - energy.H2A_CURRENT_YEAR)) - 1,
             ccs_eff_loss = if_else(year <= energy.H2A_CURRENT_YEAR, ccs_eff_loss[year == energy.H2A_CURRENT_YEAR],
                                    ccs_eff_loss[year == energy.H2A_CURRENT_YEAR]*(1 + improvement_rate) ^ (year - energy.H2A_CURRENT_YEAR)),
             units = first(na.omit(units))) %>%
    ...
    for(i in unique(H2Fast_electrolyzer_NEcost_CF$Scen)){
      for(j in unique(H2Fast_electrolyzer_NEcost_CF$Year)){
        ...
        linear_model_params <- tibble(
          Scen = i,
          Year = j,
          Slope = lm(data_filtered$NEcost_2020USD_per_kg ~ data_filtered$IdleRatio)$coefficients[1],
          Intercept = lm(data_filtered$NEcost_2020USD_per_kg ~ data_filtered$IdleRatio)$coefficients[2]
        )
    ...
    L125.GlobalTechCapital_OMfixed_nuclear_elec <- L223.GlobalTechCapital_elec %>%
      ...
      mutate(sector.name = 'H2 central production',
             ...
             capacity.factor = if_else(year <= energy.H2A_CURRENT_YEAR,cap_factor_current,
    ...
    ##### Substitute H2TEA data for NE cost and IO coefs
    ...
    H2ALite_TEAdata %>%
      ...
      rename(TechnologyH2A=Technology,
             year=Year) %>%
      complete(Scenario, TechnologyH2A, year=MODEL_FUTURE_YEARS) %>%
      fill(`Energy-free levelized cost [2022$/kg]`, contains('Energy use'), .direction="down") %>%
      filter(year!=2020) ->temp

    H2ALite_TEAcosts<- temp %>%
      mutate(H2A_NE_cost=`Energy-free levelized cost [2022$/kg]`*gdp_deflator(1975,2022)/CONV_GJ_KGH2) %>%
      select(year, Scenario, TechnologyH2A,H2A_NE_cost) %>%
      inner_join(H2ALite_TEA_mapping)

    hybrid_cost<-filter(H2ALite_TEAcosts, TechnologyH2A=="Hybrid PEM") %>%
      rename(cost=H2A_NE_cost) %>%
      select(Scenario,sector.name, subsector.name, technology,year, cost) %>%
      complete(Scenario, sector.name, subsector.name, technology,  year=c(MODEL_BASE_YEARS, 2020)) %>%
      mutate(minicam.non.energy.input="electrolyzer", units="$1975/GJ H2") %>%
      group_by(Scenario) %>%
      mutate(cost = approx_fun(year, cost,  rule = 2)) %>%
      ungroup()

    L125.globaltech_cost %>%
      left_join(filter(H2ALite_TEAcosts, Scenario=="low")%>% select(-Scenario)) %>%
      mutate(cost=ifelse(!is.na(H2A_NE_cost), H2A_NE_cost, cost)) %>%
      select(-TechnologyH2A, -H2A_NE_cost) %>%
      rbind(filter(hybrid_cost, Scenario=="low") %>% select(-Scenario))->
      L125.globaltech_cost_adv

    L125.globaltech_cost %>%
      left_join(filter(H2ALite_TEAcosts, Scenario=="high")) %>%
      mutate(cost=ifelse(!is.na(H2A_NE_cost), H2A_NE_cost, cost)) %>%
      select(-Scenario, -TechnologyH2A, -H2A_NE_cost) %>%
      rbind(filter(hybrid_cost, Scenario=="high") %>% select(-Scenario))->
      L125.globaltech_cost_lotech

    L125.globaltech_cost %>%
      left_join(filter(H2ALite_TEAcosts, Scenario=="med")) %>%
      mutate(cost=ifelse(!is.na(H2A_NE_cost), H2A_NE_cost, cost)) %>%
      select(-Scenario, -TechnologyH2A, -H2A_NE_cost) %>%
      rbind(filter(hybrid_cost, Scenario=="med") %>% select(-Scenario))->
      L125.globaltech_cost



    H2ALite_TEA_IO<- temp %>%
      mutate(`regional natural gas`=`Energy use Natural Gas (Industrial) [mmBTU HHV/kg]`*HHVtoLHV * CONV_BTU_KJ / CONV_GJ_KGH2)  %>%
      mutate(`regional biomass`=`Energy use Biomass [short dry ton/kg]`/ CONV_T_METRIC_SHORT * aglu.BIO_ENERGY_CONTENT_GJT / CONV_GJ_KGH2)%>%
      mutate(elect_td_ind=`Energy use Electricity (Industrial) [kWh/kg]`* CONV_KWH_GJ / CONV_GJ_KGH2) %>%
      mutate(`global solar resource`=`Energy use Electricity (Solar) [kWh/kg]`* CONV_KWH_GJ / CONV_GJ_KGH2) %>%
      ...
      tidyr::gather(key="minicam.energy.input", value="H2A_IO", -year, -Scenario, -TechnologyH2A) %>%
      mutate(units="GJ input / GJ H2") %>%
      inner_join(H2ALite_TEA_mapping)

    hybrid_water<-filter(L125.globaltech_coef,
                         subsector.name=='electricity',
                         str_detect(minicam.energy.input, "water")) %>%
      repeat_add_columns(tibble(Scenario = unique(H2ALite_TEAdata$Scenario))) %>%
      mutate(subsector.name='hybrid')


    hybrid_coef<-filter(H2ALite_TEA_IO, TechnologyH2A=="Hybrid PEM") %>%
      select(-TechnologyH2A) %>%
      rename(value=H2A_IO) %>%
      filter(value>0) %>%
      complete(Scenario, sector.name, subsector.name, technology, minicam.energy.input, units, year=c(MODEL_BASE_YEARS, 2020)) %>%
      group_by(minicam.energy.input, Scenario) %>%
    ...
    L125.globaltech_coef%>%
      left_join(filter(H2ALite_TEA_IO, Scenario=="low")) %>%
      mutate(value=ifelse(!is.na(H2A_IO), H2A_IO, value)) %>%
      filter(value>0) %>%
      select(-Scenario, -TechnologyH2A, -H2A_IO) %>%
      rbind(filter(hybrid_coef, Scenario=="low") %>% select(-Scenario))->
      L125.globaltech_coef_adv

    L125.globaltech_coef%>%
      left_join(filter(H2ALite_TEA_IO, Scenario=="high")) %>%
      mutate(value=ifelse(!is.na(H2A_IO), H2A_IO, value)) %>%
      filter(value>0) %>%
      select(-Scenario, -TechnologyH2A, -H2A_IO) %>%
      rbind(filter(hybrid_coef, Scenario=="high") %>% select(-Scenario))->
      L125.globaltech_coef_lotech

    L125.globaltech_coef%>%
      left_join(filter(H2ALite_TEA_IO, Scenario=="med")) %>%
      mutate(value=ifelse(!is.na(H2A_IO), H2A_IO, value)) %>%
      filter(value>0) %>%
      select(-Scenario, -TechnologyH2A, -H2A_IO) %>%
      rbind(filter(hybrid_coef, Scenario=="med") %>% select(-Scenario))->
      L125.globaltech_coef
    ...
    L125.globaltech_coef %>%
      ...
      add_precursors("energy/H2A_IO_coef_data","L223.GlobalTechEff_elec","energy/mappings/H2ALite_TEA_mapping",
                     "energy/H2ALite_TEAdata")  ->
      L125.globaltech_coef

    L125.globaltech_coef_adv %>%
      add_title("Input-output coefficients of global technologies for adv hydrogen") %>%
      add_units("Unitless") %>%
      add_comments("Interpolated original data into all model years") %>%
      add_precursors("energy/H2A_IO_coef_data","L223.GlobalTechEff_elec","energy/mappings/H2ALite_TEA_mapping",
                     "energy/H2ALite_TEAdata")  ->
    ...
    L125.globaltech_coef_lotech %>%
      add_title("Input-output coefficients of global technologies for low tech hydrogen") %>%
      add_units("Unitless") %>%
      add_comments("Interpolated original data into all model years") %>%
      add_precursors("energy/H2A_IO_coef_data","L223.GlobalTechEff_elec","energy/mappings/H2ALite_TEA_mapping",
                     "energy/H2ALite_TEAdata")  ->
    ...
    L125.globaltech_cost %>%
      ...
      add_precursors("energy/H2A_NE_cost_data","L223.GlobalTechCapital_elec","L223.GlobalTechOMvar_elec",
                     "L223.GlobalTechOMfixed_elec", "L223.GlobalTechCapFac_elec","energy/mappings/H2ALite_TEA_mapping",
                     "energy/H2ALite_TEAdata")  ->
      L125.globaltech_cost

    L125.globaltech_cost_adv %>%
      add_title("Costs of global technologies for adv hydrogen") %>%
      add_units("Unitless") %>%
      add_comments("Interpolated orginal data into all model years") %>%
      add_legacy_name("L225.GlobalTechCost_h2") %>%
      add_precursors("energy/H2A_NE_cost_data","L223.GlobalTechCapital_elec","L223.GlobalTechOMvar_elec",
    ...
    L125.globaltech_cost_lotech %>%
      add_title("Costs of global technologies for low tech hydrogen") %>%
      add_units("Unitless") %>%
      add_comments("Interpolated orginal data into all model years") %>%
      add_legacy_name("L225.GlobalTechCost_h2") %>%
      add_precursors("energy/H2A_NE_cost_data","L223.GlobalTechCapital_elec","L223.GlobalTechOMvar_elec",
    ...
    return_data(L125.globaltech_coef, L125.globaltech_coef_adv, L125.globaltech_coef_lotech,
                L125.globaltech_cost, L125.globaltech_cost_adv, L125.globaltech_cost_lotech,
                L125.Electrolyzer_IdleRatio_Params)
  } else {
