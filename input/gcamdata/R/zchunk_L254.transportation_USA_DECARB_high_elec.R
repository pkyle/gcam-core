...
#' module_gcamusa_L254.transportation_USA_DECARB_high_elec
#'
...
#' the generated outputs: \code{L254.DeleteSupplysector_USAtrn_high_elec}, \code{L254.DeleteFinalDemand_USAtrn_high_elec},
#' \code{L254.Supplysector_trn_USA_high_elec}, \code{L254.FinalEnergyKeyword_trn_USA_high_elec}, \code{L254.tranSubsectorLogit_USA_high_elec},
#' \code{L254.tranSubsectorShrwtFllt_USA_high_elec}, \code{L254.tranSubsectorInterp_USA_high_elec}, \code{L254.tranSubsectorSpeed_USA_high_elec},
#' \code{L254.tranSubsectorSpeed_passthru_USA_high_elec}, \code{L254.tranSubsectorSpeed_noVOTT_USA_high_elec},
#' \code{L254.tranSubsectorSpeed_nonmotor_USA_high_elec}, \code{L254.tranSubsectorVOTT_USA_high_elec}, \code{L254.tranSubsectorFuelPref_USA_high_elec},
#' \code{L254.StubTranTech_USA_high_elec}, \code{L254.StubTranTech_passthru_USA_high_elec}, \code{L254.StubTranTech_nonmotor_USA_high_elec},
#' \code{L254.StubTranTechLoadFactor_USA_high_elec}, \code{L254.StubTranTechCost_USA_high_elec}, \code{L254.StubTranTechCoef_USA_high_elec},
#' \code{L254.PerCapitaBased_trn_USA_high_elec}, \code{L254.PriceElasticity_trn_USA_high_elec}, \code{L254.IncomeElasticity_trn_USA_high_elec},
#' \code{L254.StubTranTechCalInput_USA_high_elec}, \code{L254.StubTranTechProd_nonmotor_USA_high_elec}, \code{L254.StubTranTechCalInput_passthru_USA_high_elec},
#' \code{L254.BaseService_trn_USA_high_elec}.
...
#' @author RC Oct 2017
module_gcamusa_L254.transportation_USA_DECARB_high_elec <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "energy/mappings/UCD_techs",
             ...
             "L254.Supplysector_trn_high_elec",
    ...
    return(c("L254.DeleteSupplysector_USAtrn_high_elec",
             ...
             "L254.StubTranTechOutput_USA_high_elec"))
    ...
    L254.Supplysector_trn <- get_data(all_data, "L254.Supplysector_trn_high_elec",strip_attributes = TRUE) %>% filter(sce %in% c("CORE"))
    L254.FinalEnergyKeyword_trn <- get_data(all_data, "L254.FinalEnergyKeyword_trn_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.tranSubsectorLogit <- get_data(all_data, "L254.tranSubsectorLogit_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.tranSubsectorShrwtFllt <- get_data(all_data, "L254.tranSubsectorShrwtFllt_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.tranSubsectorInterp <- get_data(all_data, "L254.tranSubsectorInterp_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.tranSubsectorSpeed <- get_data(all_data, "L254.tranSubsectorSpeed_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.tranSubsectorSpeed_passthru <- get_data(all_data, "L254.tranSubsectorSpeed_passthru_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.tranSubsectorSpeed_noVOTT <- get_data(all_data, "L254.tranSubsectorSpeed_noVOTT_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.tranSubsectorSpeed_nonmotor <- get_data(all_data, "L254.tranSubsectorSpeed_nonmotor_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.tranSubsectorVOTT <- get_data(all_data, "L254.tranSubsectorVOTT_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.tranSubsectorFuelPref <- get_data(all_data, "L254.tranSubsectorFuelPref_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.StubTranTech <- get_data(all_data, "L254.StubTranTech_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.StubTech_passthru <- get_data(all_data, "L254.StubTech_passthru_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.StubTech_nonmotor <- get_data(all_data, "L254.StubTech_nonmotor_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.StubTranTechLoadFactor <- get_data(all_data, "L254.StubTranTechLoadFactor_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.StubTranTechCost <- get_data(all_data, "L254.StubTranTechCost_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.StubTranTechCoef <- get_data(all_data, "L254.StubTranTechCoef_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.PerCapitaBased_trn <- get_data(all_data, "L254.PerCapitaBased_trn_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.PriceElasticity_trn <- get_data(all_data, "L254.PriceElasticity_trn_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    L254.IncomeElasticity_trn <- get_data(all_data, "L254.IncomeElasticity_trn_high_elec",strip_attributes = TRUE)%>% filter(sce %in% c("CORE"))
    ...
    # L254.DeleteSupplysector_USAtrn_high_elec: Delete transportation supplysectors of the USA region
    L254.Supplysector_trn %>%
      mutate(region = region) %>% # strip off attributes like title, etc.
      filter(region == gcam.USA_REGION) %>%
      select(region, supplysector) ->
      L254.DeleteSupplysector_USAtrn_high_elec

    # L254.DeleteFinalDemand_USAtrn_high_elec: Delete energy final demand sectors of the USA region
    ...
    process_USA_to_states(L254.Supplysector_trn) -> L254.Supplysector_trn_USA_high_elec
    process_USA_to_states(L254.FinalEnergyKeyword_trn) -> L254.FinalEnergyKeyword_trn_USA_high_elec
    process_USA_to_states(L254.tranSubsectorLogit) -> L254.tranSubsectorLogit_USA_high_elec
    process_USA_to_states(L254.tranSubsectorShrwtFllt) -> L254.tranSubsectorShrwtFllt_USA_high_elec
    process_USA_to_states(L254.tranSubsectorInterp) -> L254.tranSubsectorInterp_USA_high_elec

    process_USA_to_states(L254.tranSubsectorSpeed) %>%
      # parent file (L254.tranSubsectorSpeed) contains duplicate values... remove these
      distinct()-> L254.tranSubsectorSpeed_USA_high_elec

    process_USA_to_states(L254.tranSubsectorSpeed_passthru) -> L254.tranSubsectorSpeed_passthru_USA_high_elec
    process_USA_to_states(L254.tranSubsectorSpeed_noVOTT) -> L254.tranSubsectorSpeed_noVOTT_USA_high_elec
    process_USA_to_states(L254.tranSubsectorSpeed_nonmotor) -> L254.tranSubsectorSpeed_nonmotor_USA_high_elec
    process_USA_to_states(L254.tranSubsectorVOTT) -> L254.tranSubsectorVOTT_USA_high_elec
    process_USA_to_states(L254.tranSubsectorFuelPref) -> L254.tranSubsectorFuelPref_USA_high_elec
    process_USA_to_states(L254.StubTranTech) -> L254.StubTranTech_USA_high_elec
    process_USA_to_states(L254.StubTech_passthru) -> L254.StubTranTech_passthru_USA_high_elec
    process_USA_to_states(L254.StubTech_nonmotor) -> L254.StubTranTech_nonmotor_USA_high_elec
    process_USA_to_states(L254.StubTranTechLoadFactor) -> L254.StubTranTechLoadFactor_USA_high_elec
    process_USA_to_states(L254.StubTranTechCost) -> L254.StubTranTechCost_USA_high_elec
    ...
    process_USA_to_states(L254.PerCapitaBased_trn) -> L254.PerCapitaBased_trn_USA_high_elec
    process_USA_to_states(L254.PriceElasticity_trn) -> L254.PriceElasticity_trn_USA_high_elec
    process_USA_to_states(L254.IncomeElasticity_trn) -> L254.IncomeElasticity_trn_USA_high_elec

    # Calibration
    # L254.StubTranTechCalInput_USA_high_elec: calibrated energy consumption by all technologies
    ...
    L254.StubTranTechCoef_USA_high_elec %>%
      ...
      left_join(L254.StubTranTechCalInput_USA_high_elec,
    ...
    # L254.StubTranTechProd_nonmotor_USA_high_elec: service output of non-motorized transportation technologies
    L154.out_mpkm_state_trn_nonmotor_Yh %>%
      filter(year %in% MODEL_BASE_YEARS) %>%
    ...
    # L254.StubTranTechCalInput_passthru_USA_high_elec: calibrated input of passthrough technologies
    ...
    L254.StubTranTechCalInput_USA_high_elec %>%
      left_join_error_no_match(L254.StubTranTechLoadFactor_USA_high_elec %>% filter(sce=="CORE"),
                               by = c("region", "supplysector", "tranSubsector", "stub.technology", "year","sce")) %>%
      left_join_error_no_match(L254.StubTranTechCoef_USA_high_elec%>% filter(sce=="CORE"),
                               by = c("region", "supplysector", "tranSubsector", "stub.technology", "year", "minicam.energy.input","sce")) %>%
    ...
    L254.StubTranTechOutput_USA_high_elec %>%
      ...
      ungroup() ->
      L254.StubTranTechOutput_USA_high_elec_agg
    ...
    A54.globaltech_passthru %>%
      ...
      semi_join(L254.StubTranTech_passthru_USA_high_elec,
      ...
      left_join(L254.StubTranTechOutput_USA_high_elec_agg,
      ...
      ungroup() ->
      L254.StubTranTechCalInput_passthru_USA_high_elec_cum

    # Prepare a list of the supplysector in the passthrough input table to filter the sub-total sectors
    LIST_supplysector <- unique(L254.StubTranTechCalInput_passthru_USA_high_elec_cum$supplysector)
    ...
    # L254.BaseService_trn_USA_high_elec: base-year service output of transportation final demand
    L254.StubTranTechOutput_USA_high_elec %>%
      select(LEVEL2_DATA_NAMES[["StubTranTech"]], year, base.service = output) %>%
      bind_rows(L254.StubTranTechProd_nonmotor_USA_high_elec %>%
    ...
    L254.DeleteFinalDemand_USAtrn_high_elec %>%
      ...
      add_precursors("L254.PerCapitaBased_trn_high_elec") ->
      L254.DeleteFinalDemand_USAtrn_high_elec
    ...
    L254.StubTranTechCalInput_USA_high_elec %>%
      ...
      same_precursors_as("L254.StubTranTechCoef_USA_high_elec") %>%
    ...
    L254.StubTranTechCalInput_passthru_USA_high_elec %>%
      ...
      same_precursors_as("L254.StubTranTechCalInput_USA_high_elec") %>%
      same_precursors_as("L254.StubTranTechLoadFactor_USA_high_elec") %>%
      same_precursors_as("L254.StubTranTechCoef_USA_high_elec") %>%
      same_precursors_as("L254.StubTranTech_passthru_USA_high_elec") %>%
    ...
    L254.BaseService_trn_USA_high_elec %>%
      ...
      same_precursors_as("L254.StubTranTechLoadFactor_USA_high_elec") %>%
      same_precursors_as("L254.StubTranTechCoef_USA_high_elec") %>%
      same_precursors_as("L254.StubTranTechProd_nonmotor_USA_high_elec") %>%
    ...
    return_data(L254.DeleteSupplysector_USAtrn_high_elec, L254.DeleteFinalDemand_USAtrn_high_elec,
                L254.Supplysector_trn_USA_high_elec,
                L254.FinalEnergyKeyword_trn_USA_high_elec,
                L254.tranSubsectorLogit_USA_high_elec,
                L254.tranSubsectorShrwtFllt_USA_high_elec,
                ...
                L254.PriceElasticity_trn_USA_high_elec,
                L254.IncomeElasticity_trn_USA_high_elec,
                L254.StubTranTechCalInput_USA_high_elec, L254.StubTranTechProd_nonmotor_USA_high_elec,
                L254.StubTranTechCalInput_passthru_USA_high_elec, L254.BaseService_trn_USA_high_elec,
                L254.StubTranTechOutput_USA_high_elec)
  } else {
