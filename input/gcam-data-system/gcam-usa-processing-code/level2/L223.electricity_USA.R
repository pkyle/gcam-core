...
for( i in 1:length( L223.tables ) ){
  if( !is.null( L223.tables[[i]] ) ){
    ...
    #The script rewrites the CSV files (L223.SubsectorShrwt_renew_USA, L223.SubsectorInterp_elec_USA, L223.SubsectorInterpTo_elec_USA, 
    #L223.SubsectorShrwt_nuc_USA) with the data in the assumption files. 
    #To improve:  1)	Create an interpoTo assumption file for fossil fuels. 2)	Improve the R script for nuclear (make it similar to geothermal)
    #             3)	Combine assumption files into 2-3 files for simplicity.
    # There may be a need to rewrite the scrip to do not overwrite as it is done here. I.e., create shrwts directly from assumptions.
    
    if(objectname=="L223.SubsectorShrwt_renew_USA"){ 
      Shrwt <- readdata( "GCAMUSA_ASSUMPTIONS", "A23.alltech_shrwt_breakvalue" , skip = 3, must.exist = F ) #Read assumption file and write to all states
      Shrwt_States <- write_to_all_states( subset( Shrwt, region == "USA" ), names( Shrwt ) )
      #delete subsectors in the original file (object). 
      object<-object[object$subsector!="solar",]
      ...
      Shrwt_States_geo<-Shrwt_States[Shrwt_States$subsector=="geothermal",]
      Shrwt_States<-Shrwt_States[Shrwt_States$subsector!="geothermal",]
      Shrwt_States_geo<-Shrwt_States_geo[!(Shrwt_States_geo$region %in% geo_states_noresource_del),]
      Shrwt_States<-rbind(Shrwt_States,Shrwt_States_geo)
      
      #Write csv with new assumptions 
      objectname<-"L223.SubsectorShrwt_alltech_USA"
    ...
    if(objectname=="L223.SubsectorInterp_elec_USA"){
      ...
      Renewables_Interp <- readdata( "GCAMUSA_ASSUMPTIONS", "A23.ren_shrwt_function" , skip = 3, must.exist = F )
      Fossil_Interp_States <- write_to_all_states( subset( Fossil_Interp, region == "USA" ), names( Fossil_Interp ) )
      Renewables_Interp_States <- write_to_all_states( subset( Renewables_Interp, region == "USA" ), names( Renewables_Interp ) )
      object<-object[object$subsector!="solar",]
      object<-object[object$subsector!="wind",]
      object<-object[object$subsector!="refined liquids",]
      object<-object[object$subsector!="biomass",]
      ...
      # Remove states with no geothermal resources
      geo_states_noresource_del<-NREL_us_re_technical_potential$state[ NREL_us_re_technical_potential$Geothermal_Hydrothermal_GWh == 0 ]
      Renewables_Interp_States_geo<-Renewables_Interp_States[Renewables_Interp_States$subsector=="geothermal",]
      Renewables_Interp_States<-Renewables_Interp_States[Renewables_Interp_States$subsector!="geothermal",]
      Renewables_Interp_States_geo<-Renewables_Interp_States_geo[!(Renewables_Interp_States_geo$region %in% geo_states_noresource_del),]
      Renewables_Interp_States<-rbind(Renewables_Interp_States,Renewables_Interp_States_geo)
      
      #Combine  fossil and renewable data from the assumption files 
      object<-rbind(object,Fossil_Interp_States)
      object<-rbind(object,Renewables_Interp_States)
      
      # We fixed the shrwt to zero for those states that did not have nuclear in 2010. Unlikely that new nuclear will be built. 
      # Read the L1 file L123.out_EJ_state_elec_F with historical data on nuclear
      # Here,  the A223.Nuclear_Interp_elec_USA_Assumptions data is passed to states with nuclear
      Have_Nuc <- readdata( "GCAMUSA_ASSUMPTIONS", "A23.nuc_shrwt_nonzerostates_function" , skip = 5, must.exist = F )
      Nuc_state <- write_to_all_states( subset( Have_Nuc, region == "USA" ), names( Have_Nuc ) )
      Nuc_hist <- readdata( "GCAMUSA_LEVEL1_DATA", "L123.out_EJ_state_elec_F")
      Nuc_status_final_year <- Nuc_hist[ Nuc_hist$fuel == "nuclear", c( state, X_final_historical_year ) ]
      colnames(Nuc_status_final_year)<-c("region", "final_year")
      Nuc_state <- merge(Nuc_state,Nuc_status_final_year,by="region")
      Nuc_state<-  Nuc_state<-Nuc_state[Nuc_state$final_year!=0,]
      Nuc_state <- subset(Nuc_state, select = -c(final_year) )
      Nuc_state<-Nuc_state[!duplicated(Nuc_state),]
    ...
    if(objectname=="L223.SubsectorInterpTo_elec_USA"){
      ...
      #Remove subsectors that are not needed
      ...
      Nuc_state <- write_to_all_states( subset( No_Nuc, region == "USA" ), names( No_Nuc ) )
      Nuc_hist <- readdata( "GCAMUSA_LEVEL1_DATA","L123.out_EJ_state_elec_F")
      Nuc_status_final_year <- Nuc_hist[ Nuc_hist$fuel == "nuclear", c( state, X_final_historical_year ) ]
      colnames(Nuc_status_final_year)<-c("region", "final_year")
      Nuc_state <- merge(Nuc_state,Nuc_status_final_year,by="region")
      Nuc_state<-Nuc_state[Nuc_state$final_year==0,]
      Nuc_state <- subset(Nuc_state, select = -c(final_year) )
      Nuc_state<-Nuc_state[!duplicated(Nuc_state),]
    ...
    #Read the shrwts used by states with nuclear. 
    if(objectname=="L223.SubsectorShrwt_nuc_USA"){
      Shrwt_elec_USA_Assumptions <- readdata( "GCAMUSA_ASSUMPTIONS", "A23.nuc_shrwt_nonzerostates_breakvalue" , must.exist = F )
      Shrwt_elec_USA_Assumptions_state <- write_to_all_states( subset( Shrwt_elec_USA_Assumptions, region == "USA" ), names( Shrwt_elec_USA_Assumptions ) )
      Nuc_hist <- readdata( "GCAMUSA_LEVEL1_DATA", "L123.out_EJ_state_elec_F" )
      
      #X_final_historical_year is used for to determine if a state had nuclear or not. (same above)
      Nuc_status_final_year <- Nuc_hist[ Nuc_hist$fuel == "nuclear", c( state, X_final_historical_year ) ]
      colnames(Nuc_status_final_year)<-c("region", "final_year")
      
      #Add information for final historical year(merge) and then remove those with no nuclear and duplicates
      Shrwt_elec_USA_Assumptions_state <- merge(Shrwt_elec_USA_Assumptions_state,Nuc_status_final_year,by="region")
      Shrwt_elec_USA_Assumptions_state<-Shrwt_elec_USA_Assumptions_state[Shrwt_elec_USA_Assumptions_state$final_year!=0,]
      Shrwt_elec_USA_Assumptions_state <- subset(Shrwt_elec_USA_Assumptions_state, select = -c(final_year) )
      Shrwt_elec_USA_Assumptions_state<-Shrwt_elec_USA_Assumptions_state[!duplicated(Shrwt_elec_USA_Assumptions_state),]
      
      #combine and write
      object<-Shrwt_elec_USA_Assumptions_state
    }
